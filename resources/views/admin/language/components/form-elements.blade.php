...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('acronym'), 'has-success': fields.acronym && fields.acronym.valid }">
    <label for="acronym" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.language.columns.acronym') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.acronym" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('acronym'), 'form-control-success': fields.acronym && fields.acronym.valid}" id="acronym" name="acronym" placeholder="{{ trans('admin.language.columns.acronym') }}">
